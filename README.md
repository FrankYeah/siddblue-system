...
`/admin` 以頁籤切換七個工具（純前端切換、不重整、各自保留狀態）：
- **💰 報價系統**：原有報價單功能。
- **💼 案件管理**：專案財務中心，**Notion 風格資料表**——每列直接顯示
  ...
  **「已提列稅金？」勾選＋備註**——收到全款後應先提列稅金再花用，避免誤當淨利；
  ...
  可**標記案件為已結案**（Modal 標頭一鍵切換），已結案案件預設從列表隱藏
  （表頭「顯示已結案」勾選可切回），🔔 催款提醒不再打擾已結案案件（但 💸 待付夥伴款仍會提醒，
  結案不代表欠夥伴的錢消失）。
- **💳 支出紀錄**：公司／個人支出總覽，**Notion 風格資料表**——每列直接顯示
  項目名稱 / 歸屬 / 分類 / 扣款週期 / 金額 / 日期 / 備註，點列彈出 Modal 編輯或刪除，
  右上角「新增支出」開空白 Modal 填寫；頂部兩張統計卡自動算出**每月固定支出底線
  (Burn Rate)**——🏢 公司／🧑 個人分別加總「每月扣款」全額 + 「每年扣款」÷ 12
  （一次性、贊助/捐款不計入），手機自動垂直堆疊；篩選列可切換歸屬（全部/公司/個人）
  與分類（一次性/訂閱制/贊助/固定週期規費），手機資料表橫向滾動。
...
- **✅ 待辦清單**：四區（🔥立即處理 / ⏳稍後再說 / 🎯長期要做的事 / 🚗外出待辦），
  點「新增」加入、🗑️ 直接刪除（新項目插在最上面，不用往下捲找）；欄位為**固定寬度＋橫向
  捲動**，文字不會太容易換行；另有獨立的 **🔁 週期提醒**區塊，可設定每週／每月／每年
  或特定日期重複出現的提醒（如「每週關心學生工作進度」），依下次發生日期排序、
  顯示「今天／明天／N 天後」，到期不會自動消失，需自行確認後刪除，存於 `workspace:todos`。
- **📚 知識庫**：取代 Apple Notes 的筆記中心，**三欄側欄式版面**（仿 macOS 備忘錄）——
  左：資料夾側欄（標籤即虛擬資料夾，顯示筆記數，hover 有鉛筆可整批重新命名標籤——
  一次改名、所有用到該標籤的筆記一起更新）／中：篩選後的筆記列表／右：編輯區，
  桌機三欄同時顯示、手機收合為「資料夾 → 列表 → 內容」三步驟導覽，各步驟皆有返回鍵。
  編輯區的標籤輸入為**下拉式選單**，可從既有標籤點選、也能繼續輸入新標籤按 Enter 新增。
  三種筆記類型——**一般筆記**（Markdown、標籤、編輯/預覽切換）、**諮詢紀錄**（一鍵載入
  提問模板）、**流程知識**（報稅步驟、諮詢提問流程、網站架設說明等可重複使用的 SOP，
  改用結構化「步驟」編輯——每步驟可填標題／多行說明／多個相關連結，可增刪、上移下移
  排序）；一鍵開對外唯讀分享連結 `/shared/note/[token]`。內容編輯區（一般筆記／諮詢紀錄）
  支援**上傳圖片**（按鈕選檔、或直接貼上/拖曳圖片），經 Vercel Blob 儲存後自動插入
  Markdown 圖片語法。
- **🤝 人脈庫**：Connections CRM，**Notion 風格資料表**——每列直接顯示
  ...
  **點任一儲存格的值即套用該值篩選**（再點一次取消，可疊加），點列彈出 Modal 編輯完整欄位
  ...
  編輯 Modal 內顯示**相關案件**（這位聯絡人以夥伴身分出現在哪些案件、應付/已付金額與付款狀態），
  點列可直接跳轉到案件管理並開啟該案件——與案件管理「連過去看該人詳情」互為反向連結。
...
- **📦 資料備份與匯出**：導覽列常駐——「匯出 JSON」下載目前全部資料；「立即備份」
  ...
  會覆寫目前所有資料，需二次確認）。
- **🔍 全域搜尋框**：導覽列下方的即打即搜（寫作靈感、知識庫、案件、人脈、支出皆支援）；
...
- **後台密碼保護**：設定 `ADMIN_PASSWORD` 後 `/admin` 與寫入 API 需登入；密碼欄位可點眼睛圖示切換顯示/隱藏，避免手機打字打錯還看不到。
- **登入防暴力破解**：同一 IP 連續輸錯密碼 5 次會鎖定 15 分鐘（期間即使密碼正確也拒絕），登入成功立即清除計數，不需額外設定。
- **PWA／加到主畫面**：`/admin` 可從手機瀏覽器「加到主畫面」，以獨立視窗開啟（無網址列），品牌色系圖示與工具列色彩皆一致；刻意不做離線快取，避免看到過期資料。
...
expense:{id}     → 單筆支出紀錄；expenses:index 同上
...
app/
  ...
  api/
    ...
    expenses/route.ts        列表 / 建立
    expenses/[id]/route.ts   讀取 / 更新 / 刪除
