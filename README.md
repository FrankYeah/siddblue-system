...
- **📚 知識庫**：取代 Apple Notes 的筆記中心，三種類型——**一般筆記**（Markdown、標籤、
  編輯/預覽切換）、**諮詢紀錄**（一鍵載入提問模板）、**流程知識**（報稅步驟、諮詢提問流程、
  網站架設說明等可重複使用的 SOP，改用結構化「步驟」編輯——每步驟可填標題／多行說明／
  多個相關連結，可增刪、上移下移排序）；一鍵開對外唯讀分享連結 `/shared/note/[token]`。
  內容編輯區（一般筆記／諮詢紀錄）支援**上傳圖片**（按鈕選檔、或直接貼上/拖曳圖片），
  經 Vercel Blob 儲存後自動插入 Markdown 圖片語法。
